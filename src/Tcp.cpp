...
/*   Updated: 2026/05/11 11:46:09 by alechin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static std::string handleGET(const Request &req, const LocationConfig *loc)
{
    ...
    if (req.path.find("..") != std::string::npos)
        return errorResponse(403, "Forbidden", NULL);
                                  ...
                                  const LocationConfig *loc)
{
	...
	if (dot != std::string::npos)
	{
	    ...
	    if (ext == ".py")
	    {
	        ...
	        return executeCGI(req, scriptPath);
	    }
	}
	if (req.method == "HEAD")
	{
	    std::string response = handleGET(req, loc);
                          ...
                          time_t *lastActivity, std::map<int, const ServerConfig*> &clientToConfig)
{
	...
	if (req.body.size() > cfg->max_body_size)
	{
	    std::string response =
	        errorResponse(413, "Payload Too Large", cfg);
